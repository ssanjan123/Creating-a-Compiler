...
/*		else{
	...
	private boolean startsHighestPrecedence(Token token) {
		return startsBracketsExpression(token) || startsLiteral(token) || startsTypecastExpression(token) || startsArrayInstantiation(token) || startsparseArrayLiteralOrAccess(token) ;
	}
	...
	private boolean startsAtomicExpression(Token token) {
		...
		return  startsUnaryExpression(token) || startsBracketsExpression(token) || startsLiteral(token) || startsTypecastExpression(token) || startsArrayInstantiation(token) || startsparseArrayLiteralOrAccess(token);
	}
